...
void OctoMapper::Init() {
    ...
    map_PC_subscriber_ = node_handle_.subscribe<sensor_msgs::PointCloud2>("/map_points", 10, &OctoMapper::MapCallback,
                                                                          this);
    ORBstate_subscriber_ = node_handle_.subscribe<orb_slam2_ros::ORBState>("/info/state", 10,
                                                                           &OctoMapper::ORBStateCallback, this);
    GBAstate_subscriber_ = node_handle_.subscribe<std_msgs::Bool>("/gba_running", 10, &OctoMapper::GBAStateCallback,
                                                                  this);

    // Services
    //octoGetter_server_ = node_handle_.advertiseService(name_of_node_+"/get_octomap", &OctoMapper::GetOctomapSrv, this);
    getTfTransformMatrix(T_optical_target_, "zed2_left_camera_optical_frame", target_frame_id_param_);
...
void OctoMapper::MapCallback(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    if (orb_state_ == 1) { // ORB-SLAM is OK
        ...
        update_octomap_ = false;
        clear_octomap_ = true;
        ROS_WARN_THROTTLE(1, "%s: Not updating OctoMap if ORB-SLAM is UNKNOWN, NOT_READY, NOT_INITIALIZED.",
                          name_of_node_.c_str());
    }
...
bool OctoMapper::getTfTransformMatrix(Eigen::Affine3d &transform_matrix, const std::string source_frame,
                                      const std::string target_frame) {
    ROS_INFO("Got the function");
    try {
        ROS_INFO("Trying to find transform from %s to %s", target_frame.c_str(), source_frame.c_str());
